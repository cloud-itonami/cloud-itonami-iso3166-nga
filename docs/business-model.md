# Business Model: Independent Public-Sector Market-Entry & Procurement Compliance Service — Nigeria

## Classification

- Repository: `cloud-itonami-iso3166-nga`
- ISO 3166: `NGA` (Nigeria)
...
- Social impact: [:nigerian-content-development-access :public-spend-transparency :cross-border-friction-reduction]

## Customer

- an already-incorporated `cloud-itonami-cofog-{code}` /
  `cloud-itonami-isco-{code}` / `cloud-itonami-unspsc-{segment}` /
  `cloud-itonami-{ISIC}` operator wanting to bid on a Nigerian
  public contract
...
- registration walkthrough for the Nigeria Open Contracting Portal (NOCOPO), overseen by the Bureau of Public Procurement (BPP)
- business/tax registration checklist: a Corporate Affairs Commission (CAC) business registration; Tax Identification Number (TIN) registration with the Federal Inland Revenue Service (FIRS), plus VAT registration where applicable
- local-content / preferential-procurement navigation: the Nigerian Content Development and Monitoring Board's (NCDMB) local-content requirements, which apply mandatory Nigerian-content thresholds on qualifying (especially oil-and-gas-adjacent) public contracts
...
- this service does **not** provide legal or tax advice; characterization
  ...
  routes to Nigerian-licensed counsel or a registered agent
...
- **`cloud-itonami-cofog-{code}`**: a jurisdiction-agnostic operator
  ...
  blueprint AND this one to operate in Nigeria).
