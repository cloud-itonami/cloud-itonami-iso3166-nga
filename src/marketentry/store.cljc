(ns marketentry.store
  "SSoT for the NGA market-entry compliance actor, behind a `Store`
  ...
  every prior cloud-itonami actor in this fleet uses.
    ...
    - `DatomicStore` -- backed by `langchain.db`, a Datomic-API-compatible
                        ...
                        EDN-blob codec + event-log read/append seam
                        (ADR-2607141600 -- never a hand-rolled enc/dec*
                        two-liner).
...
(defn demo-data
  ...
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Kita Systems NG" :portal "NOCOPO/BPP"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-ng-entity? true :has-ng-entity? true
             :requires-cac? true :cac-verified? true
             :drafted? false :submitted? false
             :jurisdiction "NGA" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "NOCOPO/BPP"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-ng-entity? true :has-ng-entity? true
             :requires-cac? true :cac-verified? true
    ...
    "eng-3" {:id "eng-3" :operator "Minami Systems" :portal "NOCOPO/BPP"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             ...
             :drafted? false :submitted? false
             :jurisdiction "NGA" :status :intake}
    "eng-4" {:id "eng-4" :operator "Higashi Export" :portal "NOCOPO/BPP"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-ng-entity? true :has-ng-entity? false
             :requires-cac? true :cac-verified? true
             :drafted? false :submitted? false
             :jurisdiction "NGA" :status :intake}
    "eng-5" {:id "eng-5" :operator "Nishi Logistics" :portal "NOCOPO/BPP"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-ng-entity? true :has-ng-entity? true
             :requires-cac? true :cac-verified? false
             :drafted? false :submitted? false
             :jurisdiction "NGA" :status :intake}
    "eng-6" {:id "eng-6" :operator "Chuo Civic Tech" :portal "NOCOPO/BPP"
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
                               requires-ng-entity? has-ng-entity?
                               requires-cac? cac-verified?
                               drafted? submitted?
  ...
  (cond-> {:engagement/id id}
    ...
    (some? requires-ng-entity?)  (assoc :engagement/requires-ng-entity? requires-ng-entity?)
    (some? has-ng-entity?)       (assoc :engagement/has-ng-entity? has-ng-entity?)
    (some? requires-cac?)    (assoc :engagement/requires-cac? requires-cac?)
    (some? cac-verified?)    (assoc :engagement/cac-verified? cac-verified?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/requires-ng-entity? :engagement/has-ng-entity?
   :engagement/requires-cac? :engagement/cac-verified?
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :requires-ng-entity? (boolean (:engagement/requires-ng-entity? m))
     :has-ng-entity? (boolean (:engagement/has-ng-entity? m))
     :requires-cac? (boolean (:engagement/requires-cac? m))
     :cac-verified? (boolean (:engagement/cac-verified? m))
