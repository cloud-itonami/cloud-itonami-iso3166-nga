(ns marketentry.governor
  ...
  notion of jurisdictional procurement law, whether a Nigeria-resident
  ...
  actuation is always a human call.
    ...
    2. Evidence incomplete         -- for `:filing/draft`/
                                       ...
                                       checklist on file?
    3. Nigeria-resident-rep missing  -- for `:filing/submit`, when the
                                       engagement declares
                                       `:requires-ng-entity?
                                       true` (near-universal for Nigeria
                                       ...
                                       Grounded in Nigerian legal entity (CAC)
                                       typically required for federal awards
                                       (contracting authorities / BPP).
    ...
    (let [e (store/engagement st subject)
      ...
      (when-not (and assessment
        ...
        [{:rule :evidence-incomplete
          :detail "法域の必要書類(CAC/統一資格/NOCOPO/BPP登録/代理人確認等)が充足していない状態での提案"}]))))

(defn- ng-entity-missing-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-ng-entity? true`, INDEPENDENTLY verify
  `:has-ng-entity?` is true -- the flagship genuinely new
  check this vertical adds. CONDITIONAL on the engagement's own
  `:requires-ng-entity?` ground truth (most Nigeria public
  tenders require it; a pure-foreign-market engagement may not)."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-ng-entity? e))
                 (not (true? (:has-ng-entity? e))))
        [{:rule :ng-entity-missing
          :detail (str subject " はナイジェリア居住代理人を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn- cac-unverified-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-cac? true`, INDEPENDENTLY check
  `:cac-verified?` -- CONDITIONAL on the engagement's own
  ground truth."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-cac? e))
                 (not (true? (:cac-verified? e))))
        [{:rule :cac-unverified
          :detail (str subject " はCAC確認を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn check
  ...
  (let [hard (into []
                   (concat (spec-basis-violations request proposal)
                           ...
                           (ng-entity-missing-violations request st)
                           (engagement-fee-mismatch-violations request st)
                           (cac-unverified-violations request st)
