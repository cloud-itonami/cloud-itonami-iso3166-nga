(ns marketentry.marketentryllm
  ...
  the NGA public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "has-ng-entity?=" (:has-ng-entity? e)
                        " cac-verified?=" (:cac-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-ng-entity? e))
                              (:has-ng-entity? e))
                          (or (not (:requires-cac? e))
                              (:cac-verified? e)))
                   0.9 0.3)}))
